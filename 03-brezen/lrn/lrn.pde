...
void setup(){
  ...
  for(int i = 0 ; i < ftarget.length;i++){
    ftarget[i] = ((((int)target.charAt(i))-48.0)/64.0);
    print(ftarget[i]+" ");
  }
  println();
...
*/
  ...
  for(int i = 0 ; i < output;i++){
    Neuron n = (Neuron)neurons.get(i);
    char ch = (char)((int)(n.val*64.0+48));
    out += ch;
  }
